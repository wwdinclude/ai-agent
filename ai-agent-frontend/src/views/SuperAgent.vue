...
const sendMessage = (message) => {
  ...
  eventSource.onmessage = (event) => {
    ...
    if (data && data !== '[DONE]') {
      ...
      // 实时更新同一个气泡
      if (isFirstResponse) {
        addMessage(fullResponse, false, 'ai-answer');
    ...
    if (data === '[DONE]') {
      ...
      if (messages.value.length > 0 && !messages.value[messages.value.length - 1].isUser) {
        messages.value[messages.value.length - 1].type = 'ai-final';
      }
  ...
  eventSource.onerror = (error) => {
    ...
    if (fullResponse) {
      addMessage(fullResponse, false, 'ai-error');
    }
  }
}
